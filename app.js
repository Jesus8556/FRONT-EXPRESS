...
app.post('/login', async (req, res) => {
    try {
        ...
        const response = await axios.post('http://52.0.224.33:9000/api/login', {
...
app.get('/home/:id', authenticate, async (req, res) => {
    try {
        ...
        const empresaResponse = await axios.get(`http://52.0.224.33:9000/api/empresa/${empresaId}`);

        // Modificar la URL de la imagen para incluir el host
        const empresaDetails = {
            ...empresaResponse.data,
            imagen: 'http://52.0.224.33:9000/' + empresaResponse.data.imagen,
        };
...
app.get('/parking', authenticate, async (req, res) => {
    try {
        ...
        const parkingDetailsResponse = await axios.get('http://52.0.224.33:9000/api/parking');
        const parkingDetails = parkingDetailsResponse.data;

        // Renderizar vista de parking con detalles de la empresa y del parking
        res.render('parking', { empresaDetails, parkingDetails });
    } catch (error) {
        console.error(error);
        res.render('login-error');
    }
